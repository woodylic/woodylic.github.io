...
UI进程无法执行await httpResonse.Content.ReadAsStringAsync()，因为它已经被DoCurlAsync().Result阻塞了。这就产生了一个死锁，DoCurlAsync将无法结束。ConfigureAwait(false)配置指定await后的操作无需回到原来的context，因此解决了可能的贻畅。
